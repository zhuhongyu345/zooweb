...
public class ZkCfgManagerImpl implements ZkCfgManager {
    ...
    public boolean add(String des, String connectStr, String sessionTimeOut) {
        return jdbcTemplate.update("INSERT INTO ZK VALUES(?,?,?,?)",
                UUID.randomUUID().toString().replaceAll("-", ""), des, connectStr, sessionTimeOut) != 0;
    }
    ...
    public long count() {
        Map<String, Object> map = jdbcTemplate.queryForMap("SELECT count(id) FROM ZK");
        return (Long) map.get("count(id)");
    }
